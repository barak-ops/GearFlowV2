  ...
  const mutation = useMutation({
    ...
    onError: (error: any) => {
      if (error.code === '23505') {
        ...
        showError(`שגיאה בהוספת הפריט: ${error.message}`);
      }
    },
  });
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button>הוסף פריט חדש</Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>הוספת פריט ציוד חדש</DialogTitle>
          <DialogDescription>
            מלא את הפרטים כדי להוסיף פריט ציוד חדש למלאי.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>סטטוס התחלתי</FormLabel>
            ...
            <DialogFooter className="md:col-span-2">
              <Button type="submit" disabled={mutation.isPending}>
                {mutation.isPending ? "מוסיף..." : "הוסף פריט"}
              </Button>
            </DialogFooter>
          </form>
        </Form>
