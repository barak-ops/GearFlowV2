...
BEGIN
    ...
    RAISE LOG '[notify_storage_managers_on_new_order] New order created: order_id=%, user_id=%, warehouse_id=%', NEW.id, order_user_id, order_warehouse_id;

    IF order_warehouse_id IS NOT NULL THEN
        RAISE LOG '[notify_storage_managers_on_new_order] Searching for storage managers for warehouse_id=%', order_warehouse_id;
        ...
        LOOP
            RAISE LOG '[notify_storage_managers_on_new_order] Found storage manager: id=%, name=%', manager_profile.id, manager_profile.first_name;
            INSERT INTO public.notifications (user_id, title, message, link)
            VALUES (manager_profile.id, 'בקשת השאלה חדשה', 'התקבלה בקשת השאלה חדשה מ-' || order_user_name || ' עבור המחסן שלך.', '/orders');
        END LOOP;
    ELSE
        RAISE LOG '[notify_storage_managers_on_new_order] Order has no warehouse_id, skipping storage manager notification.';
    END IF;
